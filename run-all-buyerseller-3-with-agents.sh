...
for ((org_idx=0; org_idx<$ORG_COUNT; org_idx++)); do
    ...
    for ((person_idx=0; person_idx<$PERSON_COUNT; person_idx++)); do
        ...
        ##########################################################################
        # ✨ NEW SECTION: Invoice Credential Workflow (Jupiter Chief Sales Officer only)
        ##########################################################################
        
        # Check if this is Jupiter's Chief Sales Officer
        if [ "$PERSON_ALIAS" == "Jupiter_Chief_Sales_Officer" ]; then
            echo -e "${CYAN}      ╔═══════════════════════════════════════════════════════╗${NC}"
            echo -e "${CYAN}      ║  🧾 INVOICE CREDENTIAL WORKFLOW                      ║${NC}"
            echo -e "${CYAN}      ╚═══════════════════════════════════════════════════════╝${NC}"
            echo ""
            
            echo -e "${BLUE}      → Issuing invoice credential to Tommy Buyer Agent...${NC}"
            echo ""
            
            # Step 1: Create invoice registry if not exists
            INVOICE_REGISTRY_FILE="./task-data/${PERSON_ALIAS}-invoice-registry-info.json"
            if [ ! -f "$INVOICE_REGISTRY_FILE" ]; then
                echo -e "${BLUE}        [1/4] Creating invoice credential registry...${NC}"
                ./task-scripts/invoice/invoice-registry-create.sh "$PERSON_ALIAS" "JUPITER_SALES_REGISTRY"
            else
                echo -e "${GREEN}        [1/4] Invoice registry already exists ✓${NC}"
            fi
            
            # Step 2: Issue invoice credential to tommyBuyerAgent
            echo -e "${BLUE}        [2/4] Issuing invoice credential...${NC}"
            ...
            echo -e "${BLUE}        [3/4] Buyer agent admitting invoice...${NC}"
            ./task-scripts/invoice/invoice-acdc-admit.sh "tommyBuyerAgent"
            
            # Step 4: tommyBuyerAgent presents to Sally for verification
            echo -e "${BLUE}        [4/4] Presenting invoice to Sally verifier...${NC}"
            ./task-scripts/invoice/invoice-acdc-present.sh \
                "tommyBuyerAgent" \
                "$PERSON_ALIAS"
            
            echo ""
            echo -e "${GREEN}      ✓ Invoice credential workflow complete${NC}"
            echo ""
            
            # Display invoice summary
            INVOICE_INFO_FILE="./task-data/${PERSON_ALIAS}-invoice-credential-info.json"
            if [ -f "$INVOICE_INFO_FILE" ]; then
                ...
                echo -e "${GREEN}      📄 Invoice Summary:${NC}"
                ...
                echo -e "${GREEN}        From: Jupiter Knitting Company${NC}"
                echo -e "${GREEN}        To: Tommy Hilfiger Europe B.V.${NC}"
                echo ""
            fi
        fi
